...
function handleFeatureClick(feature, level) {
    ...
    if (level === "meso") {
        navigateTo("micro",
            ...
            "meso", numericId, `Mesorregião: ${nome}`);
    }
    else if (level === "micro") {
        navigateTo("municipio",
            (f) => f.properties.microrregiao?.micro_id == numericId,
            (m) => m.micro_id == numericId,
            "micro", numericId, `Microrregião: ${nome}`);
    }
    else if (level === "municipio") {
        navigateTo("municipio",
            (f) => f.properties.id == numericId,
            (m) => m.municipio_id == numericId,
            "municipio", numericId, `Município: ${nome}`);
    }
};
    ...
    infoCardControl.update = function (props) {
        ...
        if (props) {
            ...
            if (hoverCache && hoverCache[id]) {
                ...
                headerHtml = `<span style="font-size: 11px; color: #38bdf8; text-transform: uppercase; font-weight: bold; letter-spacing: 0.5px;">Comparando: ${label}</span><br/><h4 style="color: #ffffff; margin: 4px 0 12px 0; font-size: 16px;">${nome}</h4>`;
            } else {
                headerHtml = `<span style="font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px;">Foco em</span><br/><h4 style="color: #38bdf8; margin: 4px 0 12px 0; font-size: 16px;">${nome}</h4>`;
            }
        } else {
        ...
        const crecheTotal = (s.escolas_municipais_com_creche || 0) + (s.escolas_estaduais_com_creche || 0);
        const fundTotal = (s.escolas_municipais_com_fundamental || 0) + (s.escolas_estaduais_com_fundamental || 0);
        const medioTotal = (s.escolas_estaduais_com_medio || 0);

        this._div.innerHTML = `${headerHtml}<div class="info-row"><span style="display:flex; align-items:center;">${icon(paths.total, "#e2e8f0")} Total Escolas:</span> <b style="color: #ffffff;">${s.total_escolas}</b></div><div class="info-row"><span style="display:flex; align-items:center;">${icon(paths.urbana)} Urbanas:</span> <b>${s.total_escolas_urbanas}</b></div><div class="info-row"><span style="display:flex; align-items:center;">${icon(paths.rural)} Rurais:</span> <b>${s.total_escolas_rurais}</b></div><hr style="border-top: 1px solid #2d3748; margin: 12px 0; border-bottom: none;"/><div class="info-row"><span style="display:flex; align-items:center;">${icon(paths.creche)} Creche/Pré:</span> <b>${crecheTotal}</b></div><div class="info-row"><span style="display:flex; align-items:center;">${icon(paths.fund)} Ens. Fund:</span> <b>${fundTotal}</b></div><div class="info-row"><span style="display:flex; align-items:center;">${icon(paths.medio)} Ens. Médio:</span> <b>${medioTotal}</b></div>`;
    };
...
function renderMarkers(data) {
    ...
    data.forEach(item => {
        ...
        try {
            ...
            marker.bindPopup(`<div class="custom-dark-popup"><b style="color:#0ea5e9;">${item.escola_nome || "Escola sem nome"}</b><br/><span style="color:#a0aec0; font-size:12px;">${item.escola_endereco ?? "Endereço não disponível"}</span></div>`);
